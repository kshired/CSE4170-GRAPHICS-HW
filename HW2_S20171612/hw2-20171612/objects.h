...
void prepare_axes(void) { // Draw axes in their MC.
	axes[0][0] = -win_width / 1.5f; axes[0][1] = 0.0f;
	axes[1][0] = win_width / 1.5f; axes[1][1] = 0.0f;
	axes[2][0] = 0.0f; axes[2][1] = -win_height /1.5f;
	axes[3][0] = 0.0f; axes[3][1] = win_height / 1.5f;
...
void update_axes(void) {
	axes[0][0] = -win_width / 1.5f; axes[1][0] = win_width / 1.5f;
	axes[2][1] = -win_height / 1.5f;
	axes[3][1] = win_height / 1.5f;
...
GLfloat bg1[5][2] = { {-10.0,24.0},{-18.0,20.0},{-18.0,-20.0},{-10.0, -24.0},{3.0,0} };
GLfloat bg2[5][2] = { {-3.0, 0.0},{24.0,30.0},{30.0,22.0},{30.0,-24.0},{24.0,-33.0} };
GLfloat tri1[3][2] = { {-5.0,0.0},{-12.0,17.0},{-12.0,-18.0} };
GLfloat tri2[3][2] = { { 5.0,0.0},{ 20.0,18.0},{20.0,-17.0} };

GLfloat vs_color[4][3] = {
	{91 / 255.0f,49 / 255.0f,142 / 255.0f},
	{91 / 255.0f,49 / 255.0f,142 / 255.0f},
	{1.0f, 1.0f, 1.0f},
	{1.0f, 1.0f, 1.0f}
};

GLuint VBO_vs, VAO_vs;

void prepare_vs() {
	GLsizeiptr buffer_size = sizeof(bg1) + sizeof(bg2) + sizeof(tri1) + sizeof(tri2);

	// Initialize vertex buffer object.
	glGenBuffers(1, &VBO_vs);

	glBindBuffer(GL_ARRAY_BUFFER, VBO_vs);
	glBufferData(GL_ARRAY_BUFFER, buffer_size, NULL, GL_STATIC_DRAW); // allocate buffer object memory

	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(bg1), bg1);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(bg1), sizeof(bg2), bg2);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(bg1) + sizeof(bg2), sizeof(tri1), tri1);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(bg1) + sizeof(bg2) + sizeof(tri1), sizeof(tri2), tri2);

	// Initialize vertex array object.
	glGenVertexArrays(1, &VAO_vs);
	glBindVertexArray(VAO_vs);
...
void draw_vs() {
	...
	glUniform3fv(loc_primitive_color, 1, vs_color[0]);
	glDrawArrays(GL_TRIANGLE_FAN, 0, 5);

	glUniform3fv(loc_primitive_color, 1, vs_color[1]);
	glDrawArrays(GL_TRIANGLE_FAN, 5, 5);

	glUniform3fv(loc_primitive_color, 1, vs_color[2]);
	glDrawArrays(GL_TRIANGLE_FAN, 10, 3);


	glUniform3fv(loc_primitive_color, 1, vs_color[3]);
